...
class PokeFetch extends Component {
  ...
  updateTimer = () => {
    let internalTimerRemaining = 10;

    const countdown = () => {
      internalTimerRemaining= internalTimerRemaining -1;

      if (internalTimerRemaining<= 0){
        clearInterval(newTimer)
        this.setState({remainingTime: 0})
        return
      }

      this.setState({remainingTime: internalTimerRemaining-1})
    };
    ...
    return (
      <div className={"wrapper"}>
        ...
        <div className={"pokeWrap"}>
          ...
          />
          {this.state.remainingTime === 0 ? 
          <h1 className={"pokeName"}>{this.state.pokeName}</h1> : null}
        </div>
      </div>
    );
  }
